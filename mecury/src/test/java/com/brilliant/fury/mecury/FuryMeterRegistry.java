...
public class FuryMeterRegistry extends StepMeterRegistry {
    private final static InternalLogger log = InternalLoggerFactory.getInstance(FuryMeterRegistry.class);
    ...
    public FuryMeterRegistry() {
        this(LoggingRegistryConfig.DEFAULT, Clock.SYSTEM);
    }

    public FuryMeterRegistry(LoggingRegistryConfig config, Clock clock) {
        this(config, clock, new NamedThreadFactory("logging-metrics-publisher"), log::info, null);
    }

    private FuryMeterRegistry(LoggingRegistryConfig config, Clock clock, ThreadFactory threadFactory,
        ...
        if (config.enabled()) {
            getMeters().stream().sorted((m1, m2) -> {
                ...
                m.use(gauge -> loggingSink.accept(print.id() + " value=" + print.value(gauge.value())), counter -> {
                    ...
                    loggingSink.accept(print.id() + " count=" + count);
                }, timer -> {
    ...
    public static class Builder {
         ...
         *
         * @param meterIdPrinter
         *            printer to use for meter IDs
        ...
        public FuryMeterRegistry build() {
            return new FuryMeterRegistry(config, clock, threadFactory, loggingSink, meterIdPrinter);
        }
    }
}
